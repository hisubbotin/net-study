...
namespace DrunkFibonacci
{
    internal static class DrunkFibonacci
    {
        ...
        public static IEnumerable<int> GetDeterministicRandomSequence()
        {
            ...
            */
            Random random_ = new Random(2020);
			while(true) {
				yield return random_.Next();
			}
        }
        ...
        public static IEnumerable<int> GetDrunkFibonacci()
        {
            ...
            */
			...
			{
			int cur_element_number_ = 0;
			int prevprev = 1;
			int prev = 1;
			int cur = 1;
			Random random_ = new Random(2020);
            while(true) {
				if(cur_element_number_ == 0 )  {
					cur_element_number_++;
					yield return 1;
				}
	
				if (cur_element_number_ == 1 ) {
				    cur_element_number_++;
					yield return 1;
				}
				if (cur_element_number_ % 6 == 0) { // Каждое 6-е
					cur_element_number_++;
					int new_random_number_ = random_.Next();
					int cur_element = 300;
					// с вероятностью 1/2 зануляем биты
					if(new_random_number_ % 2 == 0) {
						cur_element = cur_element & ~42;
						// ~ это побитовое не
					}
					// все остальные
					prevprev = prev;
					prev = cur;
					cur = cur_element;
					yield return cur_element;
				
				}
				int temp_cur = prevprev + prev;
				prevprev = prev;
				prev = cur;
				cur = temp_cur;
				cur_element_number_++;
        ...
        public static IEnumerable<int> GetXoredWithLaggedItself()
        {
            ...
            return GetDrunkFibonacci().Zip(GetDrunkFibonacci().Skip(42), (x1, x2) => x1 ^ x2);
			// ^ is XOR
        }
        ...
        public static IEnumerable<int[]> GetInChunks()
        {
			...
			int chunk_number = 0;
			var chunk_ = new int[16];
			while(true) {
				int i = 0;
			    foreach(var el in GetDrunkFibonacci().Skip(chunk_number*16).Take(16).ToList())
				{
					chunk_[i] = el;
						i++;
				}
        ...
        public static Dictionary<int, int> GetGroupSizes()
        {
            ...
            */
                ...
                .GroupBy(
                    key => key % 8,
                    (key, els) => new {key, number_of_el = els.Count()})
                .ToDictionary(
					 item => item.key, item => item.number_of_el
				 );
        }
