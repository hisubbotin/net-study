    ...
    internal static class DrunkFibonacci
    {
        ...
        public static IEnumerable<int> GetDeterministicRandomSequence()
        {
            ...
            */
            //throw new NotImplementedException();
            int seed = 42;
            var cur_random = new Random(seed);

            while (true) {
                yield return cur_random.Next();
            }
        }
        ...
        public static Dictionary<int, int> GetGroupSizes()
        {
            ...
            */
            //throw new NotImplementedException();
            return GetDrunkFibonacci().Take(10000).GroupBy(x => x % 8).Select(x => (x.Key, x.Count())).ToDictionary(x => x.Item1, x => x.Item2);
        }
    }
}
