...
- GC выделяет и освобождает память

### Поколения

Все объекты в куче разделяются по 3 поколениям:

- `Generation 0` - не подвергались сборке мусора, коротко живущие объекты
- `Generation 1` - пережили одну сборку
- `Generation 2` - пережили больше одной сборки

Для оптимизации большие объекты (>85kb) складываются в отдельную кучу (Large Object Heap), которая сразу хранится во втором поколении.
Если объект выживает после сборки мусора - он переходит в следующее поколение.

При очистке мусора GC:
