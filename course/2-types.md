...
- [System.ValueType](https://msdn.microsoft.com/en-us/library/system.valuetype(v=vs.110).aspx) - наследник oject, базовый класс для всех значимых типов. Переопределяет методы object, нельзя создать его наследника напрямую. Сам он является ссылочным, но все его реалзиации - значимые.
