...
namespace AdventureTime
{
    ...
    internal static class Time
    {
        ...
        /// <param name="kind">Значение <see cref="DateTime.Kind"/>, задающий соответствующее свойство 
        /// возвращаемого объекта.</param>
        ...
        /// <summary>
        /// Конвертирует объект <see cref="DateTime"/> в эквивалентное ему строковое представление 
        /// времени в формате ISO 8601 (aka round-trip format).
        /// </summary>
        ...
        {
            /*
                Обязательно поиграйся и посмотри на изменение результата в зависимости от dt.Kind 
                (для этого тебе поможет метод выше).
                Ну и на будущее запомни этот прекрасный строковый формат представления времени - он твой бро!
                Название запоминать не нужно, просто помни, что для передачи значения в виде строки, выбирать 
                лучше инвариантные относительно сериализации/десериализации форматы.
            */
        ...
        public static DateTime ParseFromRoundTripFormat(string dtStr)
        {
            /*
                ...
                т.е. туда-обратно равно оригиналу (для туда воспользуйся предыдущим методом).
        ...
        {
            /*
                Eсли воспользуешься нужным методом, то напоминаю, 
                что результат его работы зависит от dt.Kind.
                В случае dt.Kind == Unspecified предполагается, что время локальное, 
        ...
        public static DateTime AddTenSecondsV2(DateTime dt)
        {
            /*
                ...
                что помимо конструктора, у класса есть набор полезных статических 
                методов-фабрик.
                Обрати внимание, что у TimeSpan нет статических методов FromMonth, 
                FromYear. Как думаешь, почему?
            */
        ...
        /// </summary>
        /// <remarks>
        /// Финн и Джейк, плотно поужинав, решили, что спать для слабаков и настало 
        ///  приключений, поэтому быстро собрали вещи, уложили спать БиМО и 
        /// отправились из воображаемой Москвы в воображаемый Лондон верхом на леди Ливнероге.
        /// Сколько минут они провели в пути, если Москву они покинули 28.03.2010 
        /// в 02:15 по местному времени, а в Лондон прибыли в 28.03.2010 в 02:15 по местному?
        /// </remarks>
        public static int GetAdventureTimeDurationInMinutes_ver0_Dumb()
        {
            /*
                Как ты понимаешь, время выбрано не просто так, но для начала 
                давай прикинемся совсем наивными.
                Лондон находится в часовом поясе +0 (GMT), а Москва в +3 (MSK). 
                Воспользуйся DateTimeOffset, чтобы задать правильное время, и 
                посчитай разницу в минутах. Посмотри на результат.
                Держи, заготовочку для копипасты:
                    - 2010, 3, 28, 2, 15, 0
            */

            var msc = new DateTimeOffset(2010, 3, 28, 2, 15, 0, TimeSpan.FromHours(3));
            var london = new DateTimeOffset(2010, 3, 28, 2, 15, 0, TimeSpan.FromHours(0));
            return (int)(london - msc).TotalMinutes;
        }
        ...
        /// </summary>
        /// <remarks>
        /// Фионна и Кейк, поужинав заварными пироженками от принца Жвачки, решили, 
        /// что они еще слишком молоды чтобы спать по ночам и сейчас самое время 
        /// для приключений! Дамы собрали вещи, уложили спать БиМО и отправились из 
        /// другой воображаемой Москвы в другой воображаемый Лондон верхом на лорде Монохроме.
        /// Сколько минут они провели в путешествии, если Москву они покинули 
        /// 28.03.2010 в 03:15 по местному времени, а в Лондон прибыли в 28.03.2010 в 01:15 по местному?
        /// </remarks>
        public static int GetGenderSwappedAdventureTimeDurationInMinutes_ver0_Dumb()
        {
            /*
                Здесь то же самое. Сорри, немного бездумного кодинга.
                Вот заготовочка для копипасты времени:
                    - 2010, 3, 28, 3, 15, 0
                    - 2010, 3, 28, 1, 15, 0
            */

            var msc = new DateTimeOffset(2010, 3, 28, 3, 15, 0, TimeSpan.FromHours(3));
        ...
        {
            /*
                Глава вторая, в которой оказывается, что в некоторых странах принята
                такая штука как летнее время (не совсем то, про которое поет Лана Дель Рей).

                Внимательный читатель мог усомниться в данных мной часовых поясах 
                и их смещении относительно UTC и был бы прав.
                На самом деле смещения таковы: Лондон +1 (BST - British Summer Time), 
                Москва +4 (MSD - Moscow Daylight Time).
                Давай теперь учтем правильное смещение. Я понимаю, что это очевидно,
                что результат не изменится, но тебе же не сложно скопипастить 
                и просто поменять смещения?
            */

            var msc = new DateTimeOffset(2010, 3, 28, 2, 15, 0, TimeSpan.FromHours(4));
        ...
        {
            /*
                Глава третья и последняя, в которой внезапно оказывается, 
                что Финн и Фионна находятся в суперпозиции и существуют в виде 
                гендерно нейтрального сверхчеловека, который и путешествовал из Москвы в Лондон.

                Дело в том, что перевод на летнее время в 2010м году в Москве 
                произошел в 02:00 (стрелки часов перевели на час вперед), а в Лондоне - в 01:00.
                Таким образом в Москве не было 02:15 - однако можно, например, 
                считать, что этому времени соответствует 03:15. Ну а в Лондоне 
                01:15 это на самом деле 02:15.
                Только как это обработать в рамках класса DateTimeOffset? Да,
                для конкретного примера мы могли бы сами ручками "перевести стрелки" 
                и поставить правильное время, но что делать в общем случае?
                ...
                поисках ответа (в конце концов, когда тебе это в жизни действительно понадобится),
                ниже ты найдешь готовый метод GetZonedTime. Просто посмотри на 
                него (можешь даже посмотреть методы и свойства типа TimeZoneInfo, 
                если интересно) и воспользуйся им для вычисления правильного времени
                "отбытия" и "прибытия" наших героев. Затем посчитай длительность 
            ...
            */
            ...
            var msc = GetZonedTime(new DateTime(2010, 3, 28, 2, 15, 0), moscowZoneId);
        ...
        public static int GetGenderSwappedAdventureTimeDurationInMinutes_ver2_FeelsLikeRocketScience()
        {
            /*
                ...
                действительно возвращают одно и то же время (и что оно правильное).
            */
            const string moscowZoneId = "Russian Standard Time";
            const string londonZoneId = "GMT Standard Time";

            var msc = GetZonedTime(new DateTime(2010, 3, 28, 3, 15, 0), moscowZoneId);
        ...
        private static DateTimeOffset GetZonedTime(DateTime localTime, string timeZoneId)
        {
            ...
            Console.WriteLine($"{localTime}: " +
                              "invalid = {isInvalid}; " +
                              "daylight = {isDaylightSaving}; " +
                              "ambigous = {isAmbiguous}");
        ...
        public static int GetAdventureTimeDurationInMinutes_ver3_NodaTime()
        {
            ...
            ///  есть и никаких неявных преобразований не делать.
            /// Более того, его апи не позволяет тебе сделать что-то неявно или 
            /// трактовать это время как-то иначе. Например, его невозможно 
            /// превратить в абсолютное время UTC (в Noda Time ему отвечает тип Instant)
        ...
        private static ZonedDateTime GetZonedTime(LocalDateTime localTime, string timeZoneId)
        {
            ...
            /// бросает исключение. Для наглядности конкретно этого примера 
            /// я использовал "снисходительный" вариант.
