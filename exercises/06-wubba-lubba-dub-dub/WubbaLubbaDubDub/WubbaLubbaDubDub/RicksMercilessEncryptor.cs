...
namespace WubbaLubbaDubDub
{
    public static class RicksMercilessEncryptor
    {
        ...
        public static string CharsToCodes(this string s)
        {
            ...
            */
            return string.Join("", s.Select(c => GetCode(c)));
        ...
        public static IImmutableList<long> GetUsedObjects(this string text)
        {
            ...
            */
            var id_regex = new Regex("[0-9A-F]{4}:[0-9A-F]{4}");
            var comment_regex = new Regex("\\/\\/.*\\n|\\/\\*(.|\\n)*?\\*\\/");
            return comment_regex.Split(text).SelectMany(s => id_regex.Matches(s).Select(m => m.Groups[0].Value))
