...
namespace WubbaLubbaDubDub
{
    public static class RicksMercilessEncryptor
    {
        ...
        public static string[] SplitToWords(this string line)
        {
            ...
            return
                (from result in new Regex("\\w+").Matches(line)
                    select result.Groups[0].Value).ToArray();
        }
        ...
        /// </summary>
        public static string ReplaceExtended(this string s, string old, string @new)
        {
        ...
        public static string CharsToCodes(this string s)
        {
            ...
            */
            return s.Select(GetUnicodeRepresentation).Aggregate("", (source, add) => source + add);
            
            string GetUnicodeRepresentation(char c)
            {
        ...
        public static string InverseCase(this string s)
        {
            ...
            */
            return new string(s.
                Select(c => Char.IsUpper(c) ? Char.ToLower(c) : Char.ToUpper(c)).ToArray());
        }
        ...
        {
            return new string(s.Select(c => Convert.ToChar(Convert.ToInt32(c) + 1)).ToArray());
        }
        ...
        public static IImmutableList<long> GetUsedObjects(this string text)
        {
            ...
            */
            return new Regex("\\/\\*(.|\n|\r|\r\n)*?\\*\\/|//.*(\n|\r|\r\n)").Split(text).
                SelectMany(noComments => new Regex("[0-9A-Fa-f]{4}:[0-9A-Fa-f]{4}").Matches(noComments).
                    Select(matches => matches.Groups[0].Value))
                .Select(matches => Convert.ToInt64(matches.Replace(":", String.Empty), 16)).ToImmutableList();
        }
