...
namespace CallMeMaybe
{
    public struct Maybe<T>
    {
        ...
        public Maybe<TResult> SelectMany<T2, TResult>(Func<T, Maybe<T2>> otherSelector, Func<T, T2, TResult> resultSelector)
        {
            ...
            Maybe<T2> other_maybe = otherSelector(_value);

            return other_maybe.HasValue ? resultSelector(_value, other_maybe._value) : Maybe<TResult>.Nothing;
        }
        public Maybe<TResult> SelectMany<T2, TResult>(Func<T, Maybe<T2>> otherSelector, Func<T, T2, Maybe<TResult>> maybeResultSelector)
        {
            ...
            return other_maybe.HasValue ? maybeResultSelector(_value, other_maybe._value) : Maybe<TResult>.Nothing;
        }
