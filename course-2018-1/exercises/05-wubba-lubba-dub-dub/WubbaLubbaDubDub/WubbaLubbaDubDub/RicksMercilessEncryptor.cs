...
namespace WubbaLubbaDubDub
{
    public static class RicksMercilessEncryptor
    {
        ...
        public static string InverseCase(this string s)
        {
            ...
            */
            return String.Concat(s.Select(CharInvertCase));

            char CharInvertCase(char c)
            {
        ...
        public static IImmutableList<long> GetUsedObjects(this string text)
        {
            ...
            */
            ...
            var id = new Regex("[A-F0-9]{4}:[A-F0-9]{4}");
            var textWithoutComment = comment.Split(text);

            return textWithoutComment
                     .SelectMany(x => id.Matches(x)
                     .Select(matched => Convert.ToInt64(matched.Value.Replace(":", ""))))
                     .ToImmutableList();
        }
