...
namespace WubbaLubbaDubDub
{
    public static class RicksMercilessEncryptor
    {
        ...
        public static string[] SplitToWords(this string line)
        {
            ...
            return Regex.Split(line, "[^a-zA-Z]+");
        }
        ...
        public static string CharsToCodes(this string s)
        {
            ...
            */
            ...
            {
                return $"\\u{Convert.ToInt32(ch):X4}";
            }
        ...
        public static string InverseCase(this string s)
        {
            ...
            */
            char InverseCharCase(char ch)
            {
                return char.IsUpper(ch) ? char.ToLower(ch) : char.ToUpper(ch);
            }

            return string.Concat(s.Select(InverseCharCase));
        }
        ...
        {
            string ShiftCharInc(char ch)
            {
                return char.ConvertFromUtf32(Convert.ToInt32(ch) + 1);
            }

            return string.Concat(s.Select(ShiftCharInc));
        }
        ...
        public static IImmutableList<long> GetUsedObjects(this string text)
        {
            ...
            */
            string[] noComments = Regex.Split(text, "/\\*(.|\n)*\\*/|//(.)*(\n|$)");

            Regex idRegex = new Regex("[0-9A-Fa-f]{4}:[0-9A-Fa-f]{4}");

            IEnumerable<string> SelectString(string s)
            {
                return idRegex.Matches(s).Select(m => m.Groups[0].Value);
            }

            long ConvertStringToLong(string s)
            {
                return Convert.ToInt64(s.Replace(":", ""), 16);
            }

            return noComments
                .SelectMany(SelectString)
                .Select(ConvertStringToLong)
                .ToImmutableList();
        }
