...
namespace WubbaLubbaDubDub
{
    public static class RicksMercilessEncryptor
    {
        ...
        public static string InverseCase(this string s)
        {
            ...
            */
            var newChars = s.Select(c => Char.IsLower(c) ? Char.ToUpper(c) : Char.ToLower(c));
        ...
        {
            return string.Concat(s.Select(GetNextUnicodeChar));

            string GetNextUnicodeChar(char c)
            {
                return Char.ConvertFromUtf32(c + 1);
            }
        }
        ...
        public static IImmutableList<long> GetUsedObjects(this string text)
        {
            ...
            */
            ...
            var withoutComments = commentRegex.Split(text);
            return withoutComments
                .SelectMany(s => id.Matches(s)
                    .Select(newId => newId.Value.Replace(":", "").Replace("¶", "")))
