...
namespace WubbaLubbaDubDub {
    public static class RicksMercilessEncryptor {
        ...
        public static string[] SplitToWords(this string line) {
            ...
            return Regex.Split(line, @"\s+").Where(s => s.Length > 0).ToArray();
        }
        ...
        /// </summary>
        public static string ReplaceWith(this string s, string old, string @new) {
        ...
        public static string ShiftInc(this string s) {
            return String.Concat(s.Select(c => Convert.ToChar(Convert.ToInt16(c) + 1)));
        }
        ...
        public static IImmutableList<long> GetUsedObjects(this string text) {
            ...
            */
            var textNoComments = RemoveComments(text);
            var id = new Regex(@"[0-7]{4,4}:[0-7]{4,4}");
            
            return id
                .Matches(textNoComments)
                .Select(x => x
                    .Value
                    .Where(y => y != ':')
                    .Select(y => Convert.ToInt64(y.ToString())) 
                    .Aggregate(0L, (acc, y) => acc * 8L + y))
                .ToImmutableList();
        }
