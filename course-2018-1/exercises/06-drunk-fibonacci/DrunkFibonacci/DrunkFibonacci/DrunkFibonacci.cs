...
namespace DrunkFibonacci
{
    internal static class DrunkFibonacci
    {
        ...
        public static IEnumerable<int> GetDrunkFibonacci()
        {
            ...
            */
            var i = 0;
            foreach (var tup in GetFibonacci().Zip(GetDeterministicRandomSequence(), (i1, i2) => Tuple.Create(i1, i2)))
            {
                i++;
                var val = tup.Item1;
                var randomVal = tup.Item2;
                var result = val;
                if (i % 6 == 0 && i < 24)
                {
                    continue;
        ...
        public static Dictionary<int, int> GetGroupSizes()
        {
            ...
            */
            return GetDrunkFibonacci().Take(10000).GroupBy(item => item % 8)
