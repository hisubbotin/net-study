    ...
    internal static class DrunkFibonacci
    {
        ...
        public static IEnumerable<int> GetDrunkFibonacci()
        {
            ...
            */
            ...
            var rnd = GetDeterministicRandomSequence().GetEnumerator();

            while (true)
            {
                ++index;
                var next = unchecked(previous + current);
                if ((rnd.Current & 42) == 42)
                {
                    next &= ~42;
                }
                rnd.MoveNext();
        ...
        public static IEnumerable<int[]> GetInChunks()
        {
            ...
            var gener = GetDrunkFibonacci();
            int index = 0;
            int[] chunk = new int[16];

            foreach (var number in gener)
            {
                chunk[index] = number;
                ++index;
        ...
        public static IEnumerable<int> FlattenChunkedSequence()
        {
            ...
            */
            return GetInChunks()
                   .SelectMany(chunk => chunk.OrderBy(Math.Abs).Take(3));
        }
