...
namespace AdventureTime
{
    ...
    public static class Time
    {
        ...
        public static string ToRoundTripFormatString(DateTime dt)
        {
            ...
            */
            /*у разных dt.Kind отличаются окончания строк
                dt.Local -- "+03:00"
                dt.Utc -- "Z"
                dt.Unspecified -- ""
             */
        ...
        public static DateTime AddTenSecondsV2(DateTime dt)
        {
            /*
                ...
                Обрати внимание, что у TimeSpan нет статических методов FromMonth, FromYear. Как думаешь, почему? -- разное количество дней в году и месяце
            */
        ...
        {
            /*
                1) Подумай, в чем разница между Hours и TotalHours -- Hours возвращают поле "часы" из интервала, а TotalHours возвращают полное количество часов интервала
        ...
        public static int GetTotalMinutesInThreeMonths()
        {
            ...
            var threeMonths = TimeSpan.FromDays(90); //положим, что в месяце 30 дней
            return (int) threeMonths.TotalMinutes;
        }
        ...
        public static int GetAdventureTimeDurationInMinutes_ver0_Dumb()
        {
            /*
                ...
                Держи, заготовочку для копипасты:
                    - 2010, 3, 28, 2, 15, 0
            */
            var departureTime = new DateTimeOffset(2010, 3, 28, 2, 15, 0, TimeSpan.FromHours(3));
            var arrivalTime = new DateTimeOffset(2010, 3, 28, 2, 15, 0, TimeSpan.Zero);
        ...
        public static int GetGenderSwappedAdventureTimeDurationInMinutes_ver0_Dumb()
        {
            /*
                Здесь то же самое. Сорри, немного бездумного кодинга. Вот заготовочка для копипасты времени:
                    ...
                    - 2010, 3, 28, 1, 15, 0
            */
            var departureTime = new DateTimeOffset(2010, 3, 28, 3, 15, 0, TimeSpan.FromHours(3));
            var arrivalTime = new DateTimeOffset(2010, 3, 28, 1, 15, 0, TimeSpan.Zero);
            return (int)(arrivalTime - departureTime).TotalMinutes;
        }
        ...
        public static int GetAdventureTimeDurationInMinutes_ver1_FeelsSmarter()
        {
            ...
            */
            var departureTime = new DateTimeOffset(2010, 3, 28, 2, 15, 0, TimeSpan.FromHours(4));
            var arrivalTime = new DateTimeOffset(2010, 3, 28, 2, 15, 0, TimeSpan.FromHours(1));
            return (int)(arrivalTime - departureTime).TotalMinutes;
        }
        ...
        public static int GetAdventureTimeDurationInMinutes_ver2_FeelsLikeRocketScience()
        {
            ...
            */
            ...
            var departureTime = GetZonedTime(new DateTime(2010, 3, 28, 2, 15, 0), moscowZoneId);
            var arrivalTime = GetZonedTime(new DateTime(2010, 3, 28, 2, 15, 0), londonZoneId);
            return (int)(arrivalTime - departureTime).TotalMinutes;
        }
        ...
        public static int GetGenderSwappedAdventureTimeDurationInMinutes_ver2_FeelsLikeRocketScience()
        {
            ...
            */
            const string moscowZoneId = "Russian Standard Time";
            const string londonZoneId = "GMT Standard Time";
            var departureTime = GetZonedTime(new DateTime(2010, 3, 28, 3, 15, 0), moscowZoneId);
            var arrivalTime = GetZonedTime(new DateTime(2010, 3, 28, 1, 15, 0), londonZoneId);
            return (int)(arrivalTime - departureTime).TotalMinutes;
        }
        ...
        {
            var person1BirthdayDate = person1Birthday.ToUniversalTime().Date;
            var person2BirthdayDate = person2Birthday.ToUniversalTime().Date;
            return person1BirthdayDate.Equals(person2BirthdayDate);
        }
    }
}
