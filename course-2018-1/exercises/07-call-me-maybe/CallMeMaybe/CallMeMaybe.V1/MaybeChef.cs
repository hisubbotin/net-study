...
namespace CallMeMaybe.V1 {
    public class MaybeChef : IOneRecipeChef {
        ...
        public IImmutableList<PumpkinMuffin> CookPumpkinMuffins() {
            ...
            return MakeFlourMixture()
                ...
                .Select(backingDish => oven.Bake<PumpkinBatterCup, PumpkinMuffin>(backingDish, TimeSpan.FromMinutes(30))
                    .ToMaybe()).SelectOrElse(pumpkinMuffin => pumpkinMuffin.Cups, () => null);
        }
        ...
        private Maybe<BowlOf<FlourMixture>> MakeFlourMixture() {
            return _cookingTable.FindCupsOf<WholeWheatFlour>(3.5m).ToMaybe().Select(wholeWheatFlour =>
                _cookingTable.FindCupsOf<AllPurposeFlour>(3.5m).ToMaybe().Select(allPurposeFlour =>
                    _cookingTable.FindTeaspoonsOf<PumpkinPieSpice>(5m).ToMaybe().Select(pumpkinPieSpice =>
                        _cookingTable.FindTeaspoonsOf<BakingSoda>(2m).ToMaybe().Select(bakingSoda =>
                            _cookingTable.FindTeaspoonsOf<Salt>(1.5m).ToMaybe().Select(salt =>
                                _cookingTable.FindBowlAndFillItWith(new FlourMixture()).ToMaybe())))));
        }
        ...
        private Maybe<BowlOf<EggsMixture>> MakeEggsMixture() {
            ...
            return _cookingTable.FindCansOf<PumpkingPieFilling>(1m).ToMaybe().Select(pumpkinPieFilling =>
                _cookingTable.FindCupsOf<WhiteSugar>(3m).ToMaybe().Select(sugar =>
                    _cookingTable.FindCupsOf<VegetableOil>(0.5m).ToMaybe().Select(oil =>
                        _cookingTable.FindCupsOf<Water>(0.5m).ToMaybe().Select(water =>
                            _cookingTable.FindSome<Egg>(4m).ToMaybe().Select(eggs =>
                                _cookingTable.FindBowlAndFillItWith(new EggsMixture()).ToMaybe())))));
        }
    }
}
