...
namespace CallMeMaybe.V2
{
    public class MaybeChef: IOneRecipeChef
    {
        ...
        private Maybe<BowlOf<FlourMixture>> MakeFlourMixture()
        {
            ...
            var result = from wheatFlour in _cookingTable.FindCupsOf<WholeWheatFlour>(3.5m).ToMaybe()
                         from allFlour in _cookingTable.FindCupsOf<AllPurposeFlour>(3.5m).ToMaybe()
                         from spice in _cookingTable.FindTeaspoonsOf<PumpkinPieSpice>(5m).ToMaybe()
