...
namespace WubbaLubbaDubDub
{
    public static class RicksMercilessEncryptor
    {
        ...
        {
            return Regex.Replace(line, "\\W", " ").Split(" ").Where(s => s.Length > 0).ToArray();
        }
        ...
        public static string CharsToCodes(this string s)
        {
            ...
            */
            string GetHexCode(char c)
            {
                return "\\u" + ((int) c).ToString("x4");
            }

            return string.Concat(s.Select(GetHexCode));
        }
        ...
        public static string InverseCase(this string s)
        {
            ...
            */
            char Inverce(char c)
            {
                return (Char.ToUpper(c) == c) ? Char.ToLower(c) : Char.ToUpper(c);
            }

            return String.Concat(s.Select(Inverce));
        }
        ...
        public static IImmutableList<long> GetUsedObjects(this string text)
        {
            ...
            {
                var xBites = x.Value.Where(y => y != ':' && y != '¶');
                var xBitesConverted = xBites.Select(y => Convert.ToInt64(y.ToString()));
                return Convert.ToInt64(String.Concat(xBitesConverted));
            }
            
            String withoutOneLineCommentsText = Regex.Replace(text, "//.*", "");
            String withoutCommentsText = Regex.Replace(withoutOneLineCommentsText, @"/\*(.|\n)*\*\/", "\n");
            var objectsId = Regex.Matches(withoutCommentsText, pattern: @"¶\w{4}:\w{4}¶");
            return objectsId.Select(Conversion).ToImmutableList();
        }
