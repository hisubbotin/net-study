    ...
    internal static class DrunkFibonacci
    {
        ...
        public static IEnumerable<int> GetDrunkFibonacci()
        {
            ...
            */
            ...
            while (true)
            {
                int returnVal = curVal;
                curVal = nextVal;
                nextVal = curVal + returnVal;
                random.MoveNext();
                if (curCounter % 6 == 0)
                {
                    ++curCounter;
        ...
        public static IEnumerable<int[]> GetInChunks()
        {
            ...
            var cutIter = GetDrunkFibonacci();

            while (true)
            {
                yield return cutIter.Take(16).ToArray();
                cutIter = cutIter.Skip(16);
            }
        }
        ...
        public static Dictionary<int, int> GetGroupSizes()
        {
            ...
            */
            return GetDrunkFibonacci().Take(10000)
                .GroupBy(x => (x % 8 + 8) % 8, 
                    (remainder, x) => new {Key = remainder, Value = x.Count()})
