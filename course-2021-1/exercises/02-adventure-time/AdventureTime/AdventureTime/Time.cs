...
namespace AdventureTime
{
    ...
    internal static class Time
    {
        ...
        /// </summary>
        public static int GetTotalMinutesInThreeMonths(DateTime intervalStart)
        {
            // ну тут все просто и очевидно, если сделал остальные и подумал над вопросами в комментах.
            
            // Я решил сделать три реализации:
            //      1. С указанием момента, с которого нужно считать 3 месяца
            //
            //      2. Без аргументов - тогда по умолчанию считается, что начальный момент - это текущее локальное время
            //         (это весьма спорное решение - программисту бы, наверное, хотелось, чтобы этот метод в любой точке земного
            //          шара работал одинаково, но вот пользователь, думаю, хочет считать именно начиная с текущего локального времени.
            //          А для программиста остаются 1 и 3 варианты)
            //
            //      3. С указанием начальных года и месяца
            //
            return (int) Math.Floor((intervalStart.AddMonths(3) - intervalStart).TotalMinutes);
        }
        
        public static int GetTotalMinutesInThreeMonths()
        {
            return GetTotalMinutesInThreeMonths(DateTime.Now);
        }
        
        public static int GetTotalMinutesInThreeMonths(int startYear, int startMonth)
        {
            return GetTotalMinutesInThreeMonths(new DateTime(startYear, startMonth, 0, 0, 0, 0));
        }
        ...
        public static int GetAdventureTimeDurationInMinutes_ver0_Dumb()
        {
            /*
                ...
                Держи, заготовочку для копипасты:
                    - 2010, 3, 28, 2, 15, 0
            */
            var start = new DateTimeOffset(2010, 3, 28, 2, 15, 0, TimeSpan.FromHours(3));
            var end = new DateTimeOffset(2010, 3, 28, 2, 15, 0, TimeSpan.FromHours(0));
        ...
        public static int GetGenderSwappedAdventureTimeDurationInMinutes_ver0_Dumb()
        {
            /*
                Здесь то же самое. Сорри, немного бездумного кодинга. Вот заготовочка для копипасты времени:
                    ...
                    - 2010, 3, 28, 1, 15, 0
            */
            var start = new DateTimeOffset(2010, 3, 28, 3, 15, 0, TimeSpan.FromHours(3));
            var end = new DateTimeOffset(2010, 3, 28, 1, 15, 0, TimeSpan.FromHours(0));
            return (int)(end - start).TotalMinutes;
        }
        ...
        public static int GetAdventureTimeDurationInMinutes_ver1_FeelsSmarter()
        {
            ...
            */
            var start = new DateTimeOffset(2010, 3, 28, 2, 15, 0, TimeSpan.FromHours(4));
            var end = new DateTimeOffset(2010, 3, 28, 2, 15, 0, TimeSpan.FromHours(1));
            return (int)(end - start).TotalMinutes;
        }
        ...
        public static int GetAdventureTimeDurationInMinutes_ver2_FeelsLikeRocketScience()
        {
            ...
            */
            ...
            var start = new DateTime(2010, 3, 28, 2, 15, 0);
            var end = new DateTime(2010, 3, 28, 2, 15, 0);

            var startZoned = GetZonedTime(start, moscowZoneId);
            var endZoned = GetZonedTime(end, londonZoneId);
            return (int) (endZoned - startZoned).TotalMinutes;
        }
        ...
        public static int GetGenderSwappedAdventureTimeDurationInMinutes_ver2_FeelsLikeRocketScience()
        {
            ...
            */
            ...
            var start = new DateTime(2010, 3, 28, 3, 15, 0);
            var end = new DateTime(2010, 3, 28, 1, 15, 0);
        ...
        {
            return person2Birthday.Day == person1Birthday.Day && person2Birthday.Month == person1Birthday.Month;
        }
    }
}
