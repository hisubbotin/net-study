...
namespace AdventureTime
{
    ...
    internal static class Time
    {
        ...
        public static int GetHoursBetween(DateTime dt1, DateTime dt2)
        {
            ...
            */
            // Hours - целое число от -23 до 23
            // TotalHours - целое и дробное количество часов
            // Kind объектов при арифметических операциях учитывается, поэтому, возможно, следует приводить к UTC
            // или что-то похожее, но не факт, по заданию нельзя точно определить, что требуется, поэтому...
        ...
        public static int GetTotalMinutesInThreeMonths()
        {
            ...
            // Непонятно, что за 3 месяца, есть аж 4 варианта количества дней в одном месяце
            throw new NotSupportedException();
        }
        ...
        public static int GetAdventureTimeDurationInMinutes_ver0_Dumb()
        {
            /*
                ...
                Держи, заготовочку для копипасты:
                    - 2010, 3, 28, 2, 15, 0
            */
            var startTime = new DateTimeOffset(2010, 3, 28, 2, 15, 0, TimeSpan.FromHours(3));
            var endTime = new DateTimeOffset(2010, 3, 28, 2, 15, 0, TimeSpan.FromHours(0));
        ...
        public static int GetGenderSwappedAdventureTimeDurationInMinutes_ver0_Dumb()
        {
            /*
                Здесь то же самое. Сорри, немного бездумного кодинга. Вот заготовочка для копипасты времени:
                    ...
                    - 2010, 3, 28, 1, 15, 0
            */
            var startTime = new DateTimeOffset(2010, 3, 28, 3, 15, 0, TimeSpan.FromHours(3));
            var endTime = new DateTimeOffset(2010, 3, 28, 1, 15, 0, TimeSpan.FromHours(0));
        ...
        public static int GetAdventureTimeDurationInMinutes_ver1_FeelsSmarter()
        {
            ...
            */
            // А тут о которой из задачек идёт речь? Предположим, что о последней :)
            var startTime = new DateTimeOffset(2010, 3, 28, 3, 15, 0, TimeSpan.FromHours(4));
            var endTime = new DateTimeOffset(2010, 3, 28, 1, 15, 0, TimeSpan.FromHours(1));
        ...
        public static int GetAdventureTimeDurationInMinutes_ver2_FeelsLikeRocketScience()
        {
            ...
            */
            ...
            var startTimeInMoscow = GetZonedTime(new DateTime(2010, 3, 28, 2, 15, 0), moscowZoneId);
            var endTimeInLondon = GetZonedTime(new DateTime(2010, 3, 28, 2, 15, 0), londonZoneId);

            return (int)(endTimeInLondon - startTimeInMoscow).TotalMinutes;
        }
        ...
        public static int GetGenderSwappedAdventureTimeDurationInMinutes_ver2_FeelsLikeRocketScience()
        {
            ...
            */
            ...
            var startTimeInMoscow = GetZonedTime(new DateTime(2010, 3, 28, 3, 15, 0), moscowZoneId);
            var endTimeInLondon = GetZonedTime(new DateTime(2010, 3, 28, 1, 15, 0), londonZoneId);
        ...
        {
            // Вообще было желание тоже тут кинуть NotSupportedException, потому что вообще не очевидно
            // как именно требуется сравнивать даты рождения двух людей

            if (person1Birthday.Kind == DateTimeKind.Unspecified && person2Birthday.Kind == DateTimeKind.Unspecified)
            {
                if (person1Birthday.Day == person2Birthday.Day && person1Birthday.Month == person2Birthday.Month)
                {
                    return true;
                }
                else
