...
namespace DrunkFibonacci
{
    internal static class DrunkFibonacci
    {
        ...
        public static void FillIntArray(int[] arr, int seed, int step)
        {
            ...
            for(int i = 0, acc = seed; i < arr.Length; i++, acc += step) {
                arr[i] = acc;
            }
        }
        ...
        public static IEnumerable<int> GetDrunkFibonacci()
        {
            ...
            */
            int cur = 1, prev = 0;
            byte forgetCounter = 0, jokeCounter = 1;

            int everyCount = 6;

            IEnumerable<int> rnd = GetDeterministicRandomSequence();
            foreach (int y in rnd)
            {
                int tmp = cur;
                cur = prev + cur;
                prev = tmp;
                ...
                if (forgetCounter == everyCount)
                {
                    forgetCounter = 0;
                    continue;
                }
                if(jokeCounter == everyCount)
                {
                    retval = 300;
                    jokeCounter = 0;
                }
        ...
        public static IEnumerable<int> FlattenChunkedSequence()
        {
            ...
            */
            return GetInChunks().Select(x => x.OrderBy(e => Math.Abs(e)).Take(3)).SelectMany(x => x);
        }
        ...
        public static Dictionary<int, int> GetGroupSizes()
        {
            ...
            */
            ...
            return GetDrunkFibonacci()
                .Take(count)
                .GroupBy(x => x > 0 ? x % 8 : 7 - ((-x) % 8))
                .OrderBy(grp => grp.Key)
                .ToDictionary(
                    grp => grp.Key, 
                    grp => grp.ToArray().Length
                 );
        }
    }
}
