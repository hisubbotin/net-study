...
namespace DrunkFibonacci
{
    internal static class DrunkFibonacci
    {
        ...
        public static IEnumerable<int[]> GetInChunks()
        {
            ...
            var chank = CreateIntArray(16);

            int i = 0;

            foreach (var fib in GetDrunkFibonacci())
            {
                if(i == 15)
                {
                    yield return chank;
                    chank = CreateIntArray(16);
                    i = 0;
                }
        ...
        public static Dictionary<int, int> GetGroupSizes()
        {
            ...
            */
            return GetDrunkFibonacci().Take(1000).GroupBy(x => ((x % 8 + 8) % 8)).Select(x => (x.Key, x.Count())).ToDictionary(x => x.Item1, x => x.Item2);
        }
    }
}
