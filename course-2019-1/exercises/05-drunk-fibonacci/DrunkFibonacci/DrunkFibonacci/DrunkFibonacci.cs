    ...
    internal static class DrunkFibonacci
    {
        ...
        public static IEnumerable<int> GetDrunkFibonacci()
        {
            ...
            */
            ...
            int index = 0;
            int res = 0;
            foreach (int mask in GetDeterministicRandomSequence())
            {
                int temp = first;
                first = second;
                ...
                {
                    res = 0; // Каждое 6е число забывает озвучить (но учитывает при подсчете следующего).
                    index = index % 36 + 1;
                    continue;
                }
        ...
        public static List<int> GetNextNegativeRange(int from = 1)
        {
            ...
            return GetDrunkFibonacci().Skip(from - 1).SkipWhile(elem => elem >= 0).TakeWhile(elem => elem < 0).ToList();
        }
