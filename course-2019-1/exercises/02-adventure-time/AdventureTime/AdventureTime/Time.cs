...
namespace AdventureTime
{
    ...
    internal static class Time
    {
        ...
        public static int GetHoursBetween(DateTime dt1, DateTime dt2)
        {
            ...
            */
            TimeSpan interval = ToUtc(dt2) - ToUtc(dt1);
            return (int)Math.Floor(interval.TotalHours);
        }

        /// <summary>
        /// Возвращает количество минут во временном промежутке, равном трем месяцам.
        /// </summary>
        /// <param name="firstYear">Год первого месяца.</param>
        /// <param name="firstMonth">Номер первого месяца</param>
        public static int GetTotalMinutesInThreeMonths(int firstYear, int firstMonth)
        {
            // ну тут все просто и очевидно, если сделал остальные и подумал над вопросами в комментах.
            DateTime dt1 = new DateTime(firstYear, firstMonth, 1, 0, 0, 0);
            TimeSpan interval = dt1.AddMonths(3) - dt1;
            return (int)Math.Floor(interval.TotalMinutes);
        }
        ...
        public static int GetAdventureTimeDurationInMinutes_ver0_Dumb()
        {
            /*
                ...
                Держи, заготовочку для копипасты:
                    - 2010, 3, 28, 2, 15, 0
            */
            TimeSpan msk = new TimeSpan(3, 0, 0);
            DateTimeOffset dt1 = new DateTimeOffset(2010, 3, 28, 2, 15, 0, msk);
            TimeSpan gmt = new TimeSpan(0, 0, 0);
            DateTimeOffset dt2 = new DateTimeOffset(2010, 3, 28, 2, 15, 0, gmt);
            return (int)Math.Floor((dt2 - dt1).TotalMinutes);
        }
        ...
        public static int GetGenderSwappedAdventureTimeDurationInMinutes_ver0_Dumb()
        {
            ...
            */
            TimeSpan msk = new TimeSpan(3, 0, 0);
            DateTimeOffset dt1 = new DateTimeOffset(2010, 3, 28, 3, 15, 0, msk);
            TimeSpan gmt = new TimeSpan(0, 0, 0);
            DateTimeOffset dt2 = new DateTimeOffset(2010, 3, 28, 1, 15, 0, gmt);
            return (int)Math.Floor((dt2 - dt1).TotalMinutes);
        }
        ...
        public static int GetAdventureTimeDurationInMinutes_ver1_FeelsSmarter()
        {
            ...
            */
            TimeSpan msk = new TimeSpan(4, 0, 0);
            DateTimeOffset dt1 = new DateTimeOffset(2010, 3, 28, 2, 15, 0, msk);
            TimeSpan gmt = new TimeSpan(1, 0, 0);
            DateTimeOffset dt2 = new DateTimeOffset(2010, 3, 28, 2, 15, 0, gmt);
            return (int)Math.Floor((dt2 - dt1).TotalMinutes);
        }
        ...
        public static int GetAdventureTimeDurationInMinutes_ver2_FeelsLikeRocketScience()
        {
            ...
            */
            ...
            DateTime start = new DateTime(2010, 3, 28, 2, 15, 0);
            DateTimeOffset dt1 = GetZonedTime(start, moscowZoneId);
            DateTime finish = new DateTime(2010, 3, 28, 2, 15, 0);
            DateTimeOffset dt2 = GetZonedTime(finish, londonZoneId);
            return (int)Math.Floor((dt2 - dt1).TotalMinutes);
        }
        ...
        public static int GetGenderSwappedAdventureTimeDurationInMinutes_ver2_FeelsLikeRocketScience()
        {
            ...
            */
            const string moscowZoneId = "Russian Standard Time";
            const string londonZoneId = "GMT Standard Time";
            DateTime start = new DateTime(2010, 3, 28, 3, 15, 0);
            DateTimeOffset dt1 = GetZonedTime(start, moscowZoneId);
            DateTime finish = new DateTime(2010, 3, 28, 1, 15, 0);
            DateTimeOffset dt2 = GetZonedTime(finish, londonZoneId);
            return (int)Math.Floor((dt2 - dt1).TotalMinutes);
        }
        ...
        {
            return ToUtc(person1Birthday).Date == ToUtc(person2Birthday).Date;
        }
    }
}
