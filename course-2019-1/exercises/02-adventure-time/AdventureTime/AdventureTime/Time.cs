...
namespace AdventureTime
{
    ...
    internal static class Time
    {
        ...
        public static int GetAdventureTimeDurationInMinutes_ver0_Dumb()
        {
            /*
                ...
                Держи, заготовочку для копипасты:
                    - 2010, 3, 28, 2, 15, 0
            */
            var from_datetime = new DateTimeOffset(2010, 3, 28, 2, 15, 0, TimeSpan.FromHours(3));
            var to_datetime = new DateTimeOffset(2010, 3, 28, 2, 15, 0, TimeSpan.Zero);
            var difference = to_datetime - from_datetime;
        ...
        public static int GetGenderSwappedAdventureTimeDurationInMinutes_ver0_Dumb()
        {
            /*
                Здесь то же самое. Сорри, немного бездумного кодинга. Вот заготовочка для копипасты времени:
                    ...
                    - 2010, 3, 28, 1, 15, 0
            */
            var from_datetime = new DateTimeOffset(2010, 3, 28, 3, 15, 0, TimeSpan.FromHours(3));
            var to_datetime = new DateTimeOffset(2010, 3, 28, 1, 15, 0, TimeSpan.Zero);
            var difference = to_datetime - from_datetime;
            return (int)difference.TotalMinutes;
        }
        ...
        public static int GetAdventureTimeDurationInMinutes_ver1_FeelsSmarter()
        {
            ...
            */
            var from_datetime = new DateTimeOffset(2010, 3, 28, 2, 15, 0, TimeSpan.FromHours(4));
            var to_datetime = new DateTimeOffset(2010, 3, 28, 2, 15, 0, TimeSpan.FromHours(1));
            var difference = to_datetime - from_datetime;
            return (int)difference.TotalMinutes;
        }
