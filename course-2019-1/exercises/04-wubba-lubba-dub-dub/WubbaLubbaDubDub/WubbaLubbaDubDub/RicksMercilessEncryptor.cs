...
namespace WubbaLubbaDubDub
{
    public static class RicksMercilessEncryptor
    {
        ...
        public static string CharsToCodes(this string s)
        {
            ...
            */
            return string.Concat(s.Select(c => "\\u" + ((short)c).ToString("X4")));
        ...
        public static IImmutableList<long> GetUsedObjects(this string text)
        {
            ...
            */
            ...
            var withoutComments = comments.Replace(text, " ");
            var uid = new Regex(@"([0-9A-F]{8}):([0-9A-F]{8})");
            return uid.Matches(withoutComments)
                .Select(m => (Convert.ToInt64(m.Groups[1].Value, 16) << 32) + 
