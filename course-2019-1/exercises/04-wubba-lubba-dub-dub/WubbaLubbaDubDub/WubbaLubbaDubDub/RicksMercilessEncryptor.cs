...
namespace WubbaLubbaDubDub
{
    public static class RicksMercilessEncryptor
    {
        ...
        public static string[] SplitToWords(this string line)
        {
            ...
            var rawWords = Regex.Split(line, @"[\W]+");
            return rawWords.Where(x => !string.IsNullOrEmpty(x)).ToArray();
        }
        ...
        public static string CharsToCodes(this string s)
        {
            ...
            */
            return String.Concat(s.Select(ToCharCode));

            string ToCharCode(char c)
            {
                return @"\u" + Convert.ToByte(c).ToString("x2");
            }
        }
        ...
        public static IImmutableList<long> GetUsedObjects(this string text)
        {
            ...
            */
            var cleaned = Regex.Replace(text, @"(/\*[\S\s]*?\*/|//[\S\s]*?\n)", "");
            var matches = Regex.Matches(cleaned, @"¶([0-9]{4}):([0-9]{4})¶");
            return matches.Select(match => Convert.ToInt64(String.Concat(match.Groups[1].Value, match.Groups[2].Value)))
                .ToImmutableList();
        }
